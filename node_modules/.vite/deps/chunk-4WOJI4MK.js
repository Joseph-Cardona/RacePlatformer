...
var COLLAPSIBLE_SPACES = [
  9,
  // character tabulation (tab)
  32
  // space
];
var COLLAPSIBLE_SPACES_SET = new Set(COLLAPSIBLE_SPACES);
var BREAK_AFTER_CHARS = [
  45,
  // hyphen-minus
  8208,
  // unicode hyphen
...
var BREAK_AFTER_CHARS_SET = new Set(BREAK_AFTER_CHARS);
...
function measureTaggedText(text, style, wordWrap2, context, measureTextFn, wrapMeasureTextFn, measureFontFn, canBreakCharsFn, wordWrapSplitFn) {
  var _a, _b;
  const runs = parseTaggedText(text, style);
  ...
  let maxRunStrokeWidth = ((_a = style._stroke) == null ? void 0 : _a.width) || 0;
  for (const lineRuns of wrappedRunsByLine) {
    ...
    for (const run of lineRuns) {
      ...
      lineText += run.text;
      const runStrokeWidth = ((_b = run.style._stroke) == null ? void 0 : _b.width) || 0;
...
var _CanvasTextMetrics = class _CanvasTextMetrics2 {
  ...
  static measureText(text = " ", style, canvas = _CanvasTextMetrics2._canvas, wordWrap2 = style.wordWrap) {
    ...
    if (isTagged) {
      const result = measureTaggedText(
        ...
        _CanvasTextMetrics2._measureTextAdvance,
    ...
    const width = _CanvasTextMetrics2._adjustWidthForStyle(maxLineWidth, style);
  ...
  static _measureText(text, letterSpacing, context) {
    const { metricWidth, metrics, letterSpacingVal } = _CanvasTextMetrics2._measureTextCore(text, letterSpacing, context);
  ...
  static _measureTextAdvance(text, letterSpacing, context) {
    return _CanvasTextMetrics2._measureTextCore(text, letterSpacing, context).metricWidth;
  }
  /**
  ...
  static _measureTextCore(text, letterSpacing, context) {
    ...
    if (metricWidth > 0) {
      if (useExperimentalLetterSpacing) {
        ...
        letterSpacingVal = (_CanvasTextMetrics2.graphemeSegmenter(text).length - 1) * letterSpacing;
      }
...
var _DynamicBitmapFont = class _DynamicBitmapFont2 extends AbstractBitmapFont {
  ...
  ensureCharacters(chars) {
    ...
    const extraPadding = (((_a = style.dropShadow) == null ? void 0 : _a.distance) ?? 0) + (((_b = style._stroke) == null ? void 0 : _b.width) ?? 0);
  ...
  for (let i = 0; i < chars.length + 1; i++) {
    ...
    if (isWordBreak) {
      ...
      if (char === "\r" || char === "\n") {
        ...
        const spaceWidth = charData.xAdvance + (((_a = charData.kerning) == null ? void 0 : _a[previousChar]) || 0) + adjustedLetterSpacing;
      ...
      const kerning = ((_b = charData.kerning) == null ? void 0 : _b[previousChar]) || 0;
...
var BitmapFontManagerClass = class {
  ...
  getFont(text, style) {
    ...
    if (Cache.has(fontFamilyKey)) {
      ...
      (_a = dynamicFont2.ensureCharacters) == null ? void 0 : _a.call(dynamicFont2, text);
      return dynamicFont2;
    }
    ...
    (_b = dynamicFont.ensureCharacters) == null ? void 0 : _b.call(dynamicFont, text);
    return dynamicFont;
  }
  /**
...
tiny-lru/dist/tiny-lru.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-4WOJI4MK.js.map
